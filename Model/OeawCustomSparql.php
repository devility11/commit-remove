...
class OeawCustomSparql implements OeawCustomSparqlInterface {
    ...
    public function createFullTextSparql(array $data, string $limit, string $page, bool $count = false, string $order = 'datedesc', $lang = 'en'): string{
            ...
            if(count($acdhTypes) > 0){
                ...
                foreach($td as $dtype){
                    foreach($acdhTypes as $t){
                        ...
                        if('or' == $dtype){ $or = true; continue; }

                        if( ('not' == $dtype) || ('and' == $dtype) ){
                            continue;
                        }

                        if (false !== strpos(strtolower($dtype), $val)) {
                            if(true == $or){$query .= ' UNION '; $or = false; }
        ...
        if(isset($data['years'])){
            ...
            $years = array();
            foreach ($yd as $y){
                if('or' == $y){continue; }else{ $years[] = $y;
                }
            }
            $maxYear = max($years);
            $minYear = min($years);
            $conditions .= ' ?uri <'.RC::get('drupalHasAvailableDate')."> ?date . \n";
            if (FALSE !== \DateTime::createFromFormat('Y', $maxYear) && FALSE !== \DateTime::createFromFormat('Y', $minYear)) {
