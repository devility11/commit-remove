...
class OeawFunctions {
    ...
    public function createDetailViewUrl(array $data): string {
        ...
        if(isset($data['pid']) && !empty($data['pid'])){
            if (false !== strpos($data['pid'], RC::get('epicResolver'))) {
                return $data['pid'];
            }
        }
    ...
    public function getLimitAndPageFromUrl(string $data): array {
        ...
        $limit = 10;
        foreach($data as $d) {
            if (false !== strpos($d, 'page')) {
                $page = str_replace('page=', '', $d);
            }
            if (false !== strpos($d, 'limit')) {
    ...
    public function detailViewUrlDecodeEncode(string $data, int $code = 0): string {
        ...
        if(0 == $code) {
            ...
            if (false !== strpos($identifier, 'hdl.handle.net')) {
        ...
        if(1 == $code){
            if (false !== strpos($data, 'hdl.handle.net')) {
                $data = str_replace('http://', '', $data);
            }else if(false !== strpos($data, 'https')) {
    ...
    public function pidToAcdhIdentifier(string $identifier): string {
        ...
        if(count($idsByPid) > 0){
            foreach ($idsByPid as $d){
                if (false !== strpos((string) $d['id'], RC::get('fedoraIdNamespace'))) {
                    $return = $d['id'];
                    break;
                }
            }
                    ...
                    foreach ($dissServ as $service) {
                        ...
                        if(!in_array($id, $processed)) {
                            ...
                            if($srv->getRequest($fedoraRes)->getUri()->__toString() && !empty($key)){
                                ...
                                if (false !== strpos($servUri, '/https')) {
                                    $servUri = str_replace('/https://', '/', $servUri);
                                }else if (false !== strpos($servUri, '/http')) {
                                    $servUri = str_replace('/http://', '/', $servUri);
                                }
                                if (false !== strpos($servUri, '/fcr:metadata')) {
    ...
    public function explodeSearchString(string $string): array{
        ...
        foreach($filters as $f){
            foreach($strArr as $arr){
                if (false !== strpos($arr, $f)) {
                    $arr = str_replace($f.'=', '', $arr);
                    if( ('mindate' == $f) || ('maxdate' == $f) ){
                        $arr = str_replace('+', '', $arr);
                    }
                    $res[$f] = $arr;
                }
    ...
    public function checkRules(array $rules): array{
        ...
        if(0 == count($rights) || true == $this->checkMultiDimArrayForValue('NONE', $rights)){
            throw new \Exception(
            ...
            foreach($basicPropertys as $bP) {
                if( (isset($data['table'][$bP])) && (count($data['table'][$bP]) > 0) ){
                    foreach($data['table'][$bP] as $val){
                        if('acdh:hasIdentifier' == $bP){
                            if (false == strpos($val['uri'], 'id.acdh.oeaw.ac.at')) {
                                $result['basic'][$bP][] = $val;
                            }
                        }else {
                            $result['basic'][$bP][] = $val;
    ...
    public function getFieldNewTitle(array $formElements, string $mode = 'edit'): AjaxResponse {
        ...
        if('edit' == $mode){
            ...
            foreach($formElements as $key => $value){
                if (false !== strpos($key, ':oldValues')) {
                    if(false === strpos($key, ':prop')){
            ...
            foreach($formElements as $key => $value){
                ...
                }elseif (false !== strpos($value, 'http')) {
                    $result[$key] = $value;
                }
            }
        }
        ...
        foreach($result as $key => $value){
            ...
            if (!empty($value) && false !== strpos($value, RC::get('fedoraApiUrl')) && 'file' != $key && !is_array($value)) {
        ...
        for ($x = 0; $x <= $resCount; ++$x) {
            foreach($fields as $f){
                if(!empty($result[$x]->$f)){
                    ...
                    if("EasyRdf\Resource" == $objClass){
                        $val = $result[$x]->$f;
                        $val = $val->getUri();
                        $res[$x][$f] = $val;
                    }else if("EasyRdf\Literal" == $objClass){
                        $val = $result[$x]->$f;
                        $val = $val->__toString();
                        $res[$x][$f] = $val;
                    } else {
    ...
    public function createStrongFromACDHVocabs(string $string): array {
        ...
        if (false !== strpos($string, RC::vocabsNmsp())) {
    ...
    public function createDetailViewTable(\EasyRdf\Resource $data): \Drupal\oeaw\Model\OeawResource {
        ...
        if(count($resourceIdentifiers) > 0){
            foreach ($resourceIdentifiers as $ids){
                if (false !== strpos($ids->getUri(), RC::get('fedoraUuidNamespace'))) {
                   $uuid = $ids->getUri();
                }
                $rsId[] = $ids->getUri();
            }
        ...
        foreach ($properties as $key => $val){
            if (false !== strpos($val, 'fedora.info')) {
        ...
        foreach ($properties as $p){
            ...
            foreach ($data->all($p) as $key => $val){
                if("EasyRdf\Resource" == get_class($val) ){
                    $classUri = $val->getUri();
                    if($p == RC::get('drupalRdfType')){
                        if ( (false !== strpos($val->__toString(), 'vocabs.acdh.oeaw.ac.at'))
                                &&
                                $val->localName()) 
                        {
                ...
                if( ("EasyRdf\Literal" == get_class($val)) ||
                        ("EasyRdf\Literal\DateTime" == get_class($val)) ||
                        ("EasyRdf\Literal\Integer" == get_class($val))){
                    if("EasyRdf\Literal\DateTime" == get_class($val)){
                        $dt = $val->__toString();                        
                        $time = strtotime($dt);
                    ...
                    if( ($p == RC::get('drupalEbucoreHasMime')) && (!isset($result['image'])) && (false !== strpos($val, 'image')) ) {
    ...
    public function generateCollectionData(string $id): array{
        ...
        if (false === strpos($id, RC::get('fedoraIdNamespace'))) {
            return $resData;
        }
    ...
    public function generateChildViewData(array $identifiers, \Drupal\oeaw\Model\OeawResource $data, array $properties): array{
        $result = array();
        if( (0 == count($identifiers) ) || (0 == count((array) $data) ) || (0 == count($properties)) ){
            return $result;
        }
        ...
        if('0' == $properties['limit']) { $pagelimit = '10'; } else { $pagelimit = $properties['limit']; }
    ...
    public function generateChildAPIData(string $identifier, int $limit, int $page, string $order): array {
        ...
        if(0 == count($resType)) { return array(); }
        ...
        $total = (int) count($countData);
        if('0' == $limit) { $limit = '10'; }
        if('0' != $page && '1' != $page) {  $offset = ($page - 1) * $limit; } else { $offset = 0; }
                        ...
                        if(isset($d->meta_title) && isset($d->meta_rdfType)) {
                            ...
                            if($d->meta_rdfType){
                                foreach($d->meta_rdfType as $type) {
                                    if (false !== strpos($type, RC::get('fedoraVocabsNamespace'))) {
                            ...
                            if($d->meta_identifier){
                                foreach($d->meta_identifier as $identifiers) {
                                    if (false !== strpos($identifiers, RC::get('fedoraUuidNamespace'))) {
