...
class Helper {
    ...
    public static function check3dData(array $data): bool{
        ...
        if( (isset($data['ebucore:filename'][0]))
            && 
            ( (false !== strpos(strtolower($data['ebucore:filename'][0]), '.nxs'))
            ||
            (false !== strpos(strtolower($data['ebucore:filename'][0]), '.ply')) ) 
            &&
            ( isset($data['acdh:hasCategory'][0]) && '3dData' == $data['acdh:hasCategory'][0])
    ...
    public static function generateLorisUrl(string $uri, bool $image = false): array{
        ...
        if (false !== strpos(RC::get('fedoraApiUrl'), 'hephaistos')) {
            $domain = 'hephaistos:/rest/';
        }else if(false !== strpos(RC::get('fedoraApiUrl'), 'minerva') ) {
            $domain = 'minerva:/rest/';
        }else{
         $domain = 'apollo:/rest/';
        }
        ...
        if (empty($niceURI)) {
            if (!empty($results->getTableData('acdh:hasIdentifier')) && !empty($results->getTableData('acdh:hasIdentifier')) ){
                ...
                if (isset($acdhURIs['uri'])) {
                    //id.acdh/uuid
                    if (false !== strpos($acdhURIs['uri'], 'id.acdh.oeaw.ac.at/uuid')) {
                        $niceURI = $acdhURIs['uri'];
                    }
                    //id.acdh
                    if (!isset($extras['niceURI']) && false !== strpos($acdhURIs['uri'], 'id.acdh.oeaw.ac.at')) {
                       $niceURI = $acdhURIs['uri'];
                    }
                }
                //Multiple values under acdh:hasIdentifier
                    ...
                    foreach ($acdhURIs as $key => $acdhURI) {
                        if (false !== strpos($acdhURI['uri'], 'id.acdh.oeaw.ac.at/uuid')) {
                            $acdhURIuuid = $acdhURI['uri'];
                        } else if (false !== strpos($acdhURI['uri'], 'id.acdh.oeaw.ac.at')) {
    ...
    public static function isURL(string $string): string{
        ...
        if (filter_var($string, FILTER_VALIDATE_URL)) {
            if (false !== strpos($string, RC::get('fedoraApiUrl'))) {
