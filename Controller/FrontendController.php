...
class FrontendController extends ControllerBase 
{
    ...
    public function roots_list(string $limit = '10', string $page = '1', string $order = 'datedesc' ): array {
        ...
        if(count((array) $res) > 0){
            ...
            if (empty($pageData['page']) OR 0 == $pageData['page']) {
                $datatable['#currentPage'] = 1;
            } else {
                $datatable['#currentPage'] = $pageData['page'] + 1;
            }
            if (empty($pageData) OR 0 == $pageData['totalPages']) {
    ...
    public function oeaw_detail(string $res_data) {
        ...
        if (false !== strpos($res_data, 'ajax=1')) { $response = 'ajax'; }
        ...
        */
        ...
        if( 'Collection' == $resultsObj->getType() || 'Resource' == $resultsObj->getType()
                || 'Metadata' == $resultsObj->getType() ) {
            $breadcrumbs = array();
        ...
        if(count($dissServices) > 0 && $fedoraRes->getId()){
            ...
            if('Collection' == $resultsObj->getType()){
                for($i = 0; $i <= count($dissServices); ++$i){
                    if('raw' == $dissServices[$i]['returnType']){
        ...
        if(!empty($resultsObj->getTableData('acdh:hasAvailableDate'))){
            ...
            if (FALSE !== \DateTime::createFromFormat('Y-d-d', $avDate)) {
                $time = strtotime($avDate);
                $newTime = date('Y-m-d', $time);
                if(false == $resultsObj->setTableData('acdh:hasAvailableDate', array($newTime)) || empty($newTime)){
                    drupal_set_message(t('Error').' : Available date format', 'error');
                    return array();
                }
            }
            //if we dont have a real date just a year
            if (FALSE !== \DateTime::createFromFormat('Y', $avDate)) {
                $year = \DateTime::createFromFormat('Y', $avDate);
                if(false == $resultsObj->setTableData('acdh:hasAvailableDate', array($year->format('Y')) ) || empty($year->format('Y'))){
                    drupal_set_message(t('Error').' : Available date format', 'error');
                    return array();
                }
            }
        ...
        if('Resource' == $resultsObj->getType()  ){
            if(true === Helper::check3dData($resultsObj->getTable())){
                $extras['3dData'] = true;
            }else{
                ...
                if(false !== array_search('https://id.acdh.oeaw.ac.at/dissemination/3DObject', array_column($dissServices, 'identifier'))) {
    ...
    public function oeaw_child_api(string $identifier, string $limit, string $page, string $order): Response {
        if (false === strpos($identifier, RC::get('fedoraUuidNamespace'))) {
        ...
        if(0 == count($childArray['childResult'])) {
            $childArray['errorMSG'] =  
        ...
        if ('root' == $metavalue) {
            ...
            if(
                ...
                (   (!isset($searchStr['type']))
                        ||  
                    ( isset($searchStr['type']) && 'resource' == strtolower($searchStr['type']) )
                )
            ){
            ...
            if (0 == count($result)){ return array(); }
    ...
    public function oeaw_cache_ontology(): Response {
        $result = array();
        if(true == $this->propertyTableCache->setCacheData()){
            ...
            $promise = $client->sendAsync($request)->then(function ($response) {
                if(200 == $response->getStatusCode()){
                    ...
                    if(count($response->getHeader('Content-Disposition')) > 0){
                        ...
                        if('nxs' == $extension || 'ply' == $extension){
                            if(!empty($filename)){
   ...
   public function oeaw_shibboleth_login(){
        ...
        if( (isset($_SERVER['HTTP_EPPN']) && null != $_SERVER['HTTP_EPPN'])
                && (isset($_SERVER['HTTP_AUTHORIZATION']) && null != $_SERVER['HTTP_AUTHORIZATION'])
                )
            {
